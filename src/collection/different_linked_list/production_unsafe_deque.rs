...
impl<T> LinkedList<T> {
    ...
    pub fn push_front(&mut self, elem: T) {
        unsafe {
            let new_front = NonNull::new_unchecked(Box::into_raw(Box::new(Node::new(elem))));
    ...
    pub fn push_back(&mut self, elem: T) {
        unsafe {
            let new_back = NonNull::new_unchecked(Box::into_raw(Box::new(Node::new(elem))));
...
fn assert_properties() {
    ...
    /// ```compile_fail
    /// use rust_algorithms::collection::different_linked_list::production_unsafe_deque::IterMut;
    ///
